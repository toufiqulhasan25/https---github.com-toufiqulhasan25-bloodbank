...
@section('content')
    <div class="container py-4">
        {{-- Success/Error Alerts (System Messages) --}}
        @if(session('success'))
        ...
        {{-- Notification Section (is_read লজিক অনুযায়ী শুধুমাত্র নতুন আপডেটগুলো দেখাবে) --}}
            ...
            <div class="col-md-4">
                <div class="card border-0 shadow-sm rounded-4 mb-4">
                    <div class="card-body text-center p-4">
                        ...
                        <span class="badge bg-danger-subtle text-danger px-3 py-2 rounded-pill fw-bold small">Authorized
                            Hospital</span>
                        <hr>
                    ...
                    <div class="card-body p-4">
                        ...
                        <form action="{{ route('hospital.updateStock') }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" name="bags" class="form-control border-0 bg-light rounded-3"
                                    placeholder="Ex: 5" min="0" required>
                            </div>
                            <button type="submit" class="btn btn-danger w-100 rounded-pill fw-bold shadow-sm py-2">Update
                                Inventory</button>
                        </form>
                    </div>
                </div>
            ...
            <div class="col-md-8">
                        ...
                        <div class="card border-0 shadow-sm bg-primary text-white rounded-4 overflow-hidden">
                            <div class="card-body p-4 d-flex justify-content-between align-items-center">
                                <div>
                                    ...
                                    <h2 class="fw-bold mb-0">{{ $requests->where('status', 'pending')->count() }}</h2>
                                </div>
                ...
                <div class="card border-0 shadow-sm rounded-4 mb-4">
                    <div class="card-header bg-white border-0 py-3 d-flex justify-content-between align-items-center">
                        ...
                        <span class="badge bg-light text-dark rounded-pill px-3 fw-normal border">{{ $stocks->count() }}
                            Groups</span>
                    </div>
                                ...
                                <tbody>
                                    @forelse($stocks as $stock)
                                        <tr>
                                            <td class="ps-4">
                                                <span
                                                    class="badge bg-danger text-white fw-bold px-3 py-2 rounded-3">{{ $stock->blood_group }}</span>
                                            </td>
                                ...
                                <tbody>
                                    @forelse($requests as $request)
                                        <tr>
                                            ...
                                            <td><span
                                                    class="badge bg-danger-subtle text-danger px-2 border border-danger-subtle">{{ $request->blood_group }}</span>
                                            </td>
                                            <td>
                                                ...
                                                @else
                                                    <span
                                                        class="badge bg-light text-muted px-3 py-2 rounded-pill border">{{ ucfirst($request->status) }}</span>
                                                @endif
                                            </td>
                                            <td class="pe-4 text-end"><small
                                                    class="text-muted">{{ $request->created_at->format('d M, Y') }}</small></td>
                                        </tr>
                                    @empty
                                        <tr>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (notifications.length > 0) {
        // ৫ সেকেন্ড পর ডাটাবেস আপডেট এবং এলার্ট হাইড হবে
        setTimeout(function() {
